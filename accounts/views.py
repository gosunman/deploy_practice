...
from .forms import CustumUserCreationForm, CustumUserChangForm
...
def update(request):
    if request.method == "POST":
        form = CustumUserChangForm(request.POST, instance=request.user)
        if form.is_valid():
            form.save()
            return redirect('posts:index')
    else:
        form = CustumUserChangForm(instance=request.user)
    context = {
        'form':form
    }
    return render(request, 'accounts/form.html', context)
